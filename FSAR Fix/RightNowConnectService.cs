using FSAR_Fix.RightNowService;
...
namespace FSAR_Fix
{
    class RightNowConnectService
    {
        ...
        public string[] GetIncFix()
        {
            refno = refno.Trim();
            string query = "SELECT p.ReferenceNumber, p.ID FROM Incident p WHERE  p.CustomFields.CO.ITS > 0 and p.CustomFields.CO.fsar_compliance != 1 AND p.CustomFields.c.incident_type.LookupName = 'Claim' and ID = " ;

            // string query = "SELECT p.ReferenceNumber, p.CustomFields.CO.nf_claim_nmbr FROM Incident p WHERE ID = 3404 ";
            //  MessageBox.Show(query);
            string[] resultSet = GetRNData("FSAR wrong", query);
